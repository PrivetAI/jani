...
- **Редактирование профиля**: Возможность изменить отображаемое имя, никнейм, пол и лицо персонажа (1-е/3-е)
...
### 8. Комментарии к персонажам

- **Добавление комментариев**: Пользователи могут оставлять комментарии к персонажам
- **Вложенные ответы**: Поддержка ответов на комментарии (древовидная структура)
- **Удаление своих комментариев**: Пользователь может удалить только свои комментарии
- **Отображение автора**: Имя автора комментария с датой

### 9. Создание персонажей (UGC)

- **Форма создания**: Пользователь может создать своего персонажа (имя, описание, промпт, аватар)
- **Выбор LLM модели**: Отображение рекомендуемых моделей (помеченных флагом `is_recommended`)
- **Подсказки по промптам**: Детальные советы по написанию эффективных system prompts
- **Модерация**: Созданные персонажи отправляются на модерацию админу
  - Персонаж создается с флагом `is_approved = false`
  - Не отображается в каталоге до одобрения
  - Автор может видеть свой неодобренный персонаж на странице автора
- **Одобрение/Отклонение**: Админ может одобрить или отклонить персонажа
  - При одобрении: `is_approved = true` + инвалидация кеша персонажей
  - При отклонении: персонаж удаляется из базы
- **Кнопка в профиле**: Доступ к созданию персонажа из страницы профиля

### 10. Страница автора

- **Профиль автора**: Страница со всеми персонажами конкретного автора
- **Кликабельная ссылка**: Имя автора на странице персонажа ведёт на страницу автора
- **Список персонажей**: Карточки всех одобренных персонажей автора
...
CharacterCard[
  Name("Алиса");
  Voice("3rd person", "эмоционально", "образно");  // Настраивается в профиле (1st/3rd)
...
- **Удаление персонажей** с логированием действий

### Модерация UGC персонажей
- **Очередь модерации**: просмотр всех неодобренных персонажей, созданных пользователями
- **Одобрение персонажей** (`POST /admin/characters/:id/approve`):
  - Инвалидация кеша списка персонажей
  - Логирование одобрения с деталями персонажа
  - Автоматическое уведомление в Telegram (опционально)
- **Отклонение персонажей** (`DELETE /admin/characters/:id/reject`):
  - Удаление неодобренного персонажа из базы
  - Логирование отклонения
- **Фильтрация**: отображение одобренных/неодобренных персонажей в админ-панели

### Управление LLM моделями (Allowed Models)

Система управления доступными моделями для использования в персонажах:

- **CRUD операции**:
  - Создание новых моделей с указанием провайдера (gemini/openrouter/openai)
  - Редактирование существующих моделей
  - Удаление неиспользуемых моделей
  - Активация/деактивация моделей

- **Специальные флаги**:
  - **Default Model** (`is_default`): модель по умолчанию для новых персонажей
  - **Fallback Model** (`is_fallback`): глобальная резервная модель при ошибках основной
  - **Recommended** (`is_recommended`): рекомендуемые модели для пользовательских персонажей

- **Таблица**: `allowed_models`
  ```sql
  provider: TEXT             -- 'gemini', 'openrouter', 'openai'
  model_id: TEXT UNIQUE      -- 'gemini-2.0-flash-exp'
  display_name: TEXT         -- 'Gemini 2.0 Flash'
  is_default: BOOLEAN
  is_fallback: BOOLEAN       -- Глобальный fallback при ошибках
  is_recommended: BOOLEAN    -- Показывается при создании UGC
  is_active: BOOLEAN
  ```

- **Использование**:
  - При создании персонажа админ выбирает из списка `allowed_models`
  - Пользователи видят рекомендуемые модели при создании своих персонажей
  - При ошибке LLM автоматически используется fallback модель

### Глобальные настройки приложения
- Провайдер и модель для суммаризации диалогов
- Настройка через `app_settings` таблицу
...
- Валидация типа файла через magic bytes
...
### Статистика
- Просмотр пользователей с количеством отправленных сообщений (только от пользователя)
- Метрики использования персонажей
...
- `user_character_state` — эмоциональное состояние (attraction, trust, affection, dominance)
...
- **Расписание**: Каждые 8 часов (настраивается через `BACKUP_INTERVAL`)
- **Формат**: Сжатый SQL дамп PostgreSQL
- **Доставка**: Отправка в Telegram чат (указывается в `BACKUP_CHAT_ID`)
- **Контейнер**: Отдельный Docker сервис `backup`
...
| Тариф | Стоимость | Длительность |
|-------|-----------|--------------|
| Неделя | 149 ⭐ | 7 дней |
| Месяц | 349 ⭐ | 30 дней |
| 3 месяца | 949 ⭐ | 90 дней |

**Техническая реализация:**
- Backend: `paymentService.ts` — создание invoice через `createInvoiceLink`
- Webhook: обработка `pre_checkout_query` и `successful_payment`
- Frontend: `SubscriptionPage.tsx` с `WebApp.openInvoice()`
...
| `BACKUP_CHAT_ID` | Telegram ID для отправки бэкапов | — |
| `BACKUP_INTERVAL` | Cron расписание бэкапов | `0 */8 * * *` |
| `DOMAIN` | Домен для production (Caddy SSL) | — |
